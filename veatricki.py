...
class PackhumSearchGUI:
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="Export Results to XML...", command=self.export_results_xml)
    ...
    def create_export_frame(self):
        ...
        # Filename entry
        ttk.Label(export_frame, text="Output Filename:").pack(side=tk.LEFT, padx=5)
        
        self.output_filename_var = StringVar(value="search_results_packhum")
        self.filename_entry = ttk.Entry(export_frame, textvariable=self.output_filename_var, width=30)
        self.filename_entry.pack(side=tk.LEFT, padx=5)
        
        # Extension label
        self.format_var = StringVar(value=".csv")
        self.format_label = ttk.Label(export_frame, textvariable=self.format_var, font=("Arial", 10, "bold"))
        self.format_label.pack(side=tk.LEFT, padx=5)
        
        # CSV Export button
        self.csv_button = ttk.Button(export_frame, text="💾 Export as CSV", 
                                     command=self.export_results_csv, width=15)
        self.csv_button.pack(side=tk.RIGHT, padx=5)
        
        # XML Export button
        self.xml_button = ttk.Button(export_frame, text="📄 Export as XML", 
                                     command=self.export_results_xml, width=15)
        self.xml_button.pack(side=tk.RIGHT, padx=5)
        
        # Info label
        self.export_info_label = ttk.Label(export_frame, text="", foreground="gray")
        self.export_info_label.pack(side=tk.RIGHT, padx=10)
    ...
    def on_data_loaded(self, filename):
        ...
        self.export_info_label.config(text=f"Ready to search {len(self.data):,} entries")
    ...
    def on_load_error(self, error):
        ...
        self.search_button.config(state=tk.NORMAL)
        self.export_info_label.config(text="Error loading file")
    ...
    def search(self):
        ...
        self.search_button.config(state=tk.DISABLED)
        self.export_info_label.config(text="Searching...")
    ...
    def display_results(self, results):
        ...
        self.search_button.config(state=tk.NORMAL)
        self.export_info_label.config(text=f"{len(results):,} results available for export")
...
"""
    ...
    def export_results_csv(self):
        ...
        if filename:
            try:
                ...
                self.export_info_label.config(text=f"Exported to {Path(filename).name}")
            except Exception as e:
    ...
    def clear_filters(self):
        ...
        self.export_info_label.config(text="Ready")
    
    def show_about(self):
        """Show about dialog"""
        about_text = """Welcome to Veatriki, a Packhum search tool
...
- XML: Structured format for further processing
